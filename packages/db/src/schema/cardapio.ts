import { boolean, date, numeric, pgEnum, pgTable, text, unique } from "drizzle-orm/pg-core"
...
export const prato = pgTable("prato", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  nome: text("nome").notNull(),
  categoria_id: text("categoria_id").notNull().references(() => categoria_prato.id),
})

export const prato_tamanho_preco = pgTable("prato_tamanho_preco", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
  prato_id: text("prato_id").notNull().references(() => prato.id),
...
export const cardapio_dia = pgTable("cardapio_dia", {
  id: text("id").primaryKey().$defaultFn(() => createId()),
